...
import {
  ArrowRight, CheckCircle2, Globe, Receipt, X, ChevronDown, ChevronUp,
  Server, LayoutGrid, Mail, Shield, Crown, AlertCircle, RefreshCw, Key
} from 'lucide-react';
...
const DomainTransferOrderModal = ({ isOpen, onClose, domain }) => {
  ...
  useEffect(() => {
    ...
    setFormData({ name: '', surname: '', company: '', email: '', tel: '', whatsapp: '', authCode: '' });
  }, [isOpen, domain]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (result?.success) {
      ...
      setSubmitError('Sorry, there was an error submitting your transfer request. Please try again.');
    }
  };
        ...
        >
          ...
          <div className="flex items-center justify-between p-6 border-b border-gray-100 bg-slate-50/50">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-gray-900">Complete Your Transfer</h2>
          ...
          <form className="grid grid-cols-1 lg:grid-cols-3 gap-0 overflow-hidden" onSubmit={handleSubmit}>
            ...
            <div className="lg:col-span-2 p-6 space-y-6 overflow-y-auto">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      type="text" id="surname" value={formData.surname} onChange={handleChange} required
                      className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label htmlFor="company" className="text-sm font-medium text-gray-700">Company <span className="text-gray-400 font-normal">(Optional)</span></label>
                    <input
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <input
                      type="tel" id="tel" value={formData.tel} onChange={handleChange} required
                      className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all"
                    />
                  </div>
                  <div className="space-y-1">
                    <label htmlFor="whatsapp" className="text-sm font-medium text-gray-700">WhatsApp Number <span className="text-gray-400 font-normal">(Optional)</span></label>
                    <input
                ...
                {/* Auth/EPP Code */}
                <div className="space-y-1">
                  <label htmlFor="authCode" className="text-sm font-medium text-gray-700 flex items-center gap-1.5">
                    <Key className="w-3.5 h-3.5 text-gray-400" />
                    Auth / EPP Code <span className="text-gray-400 font-normal">(Optional — you can provide this later)</span>
                  </label>
                  <input
                    type="text" id="authCode" value={formData.authCode} onChange={handleChange}
                    placeholder="Obtain this from your current registrar"
                                  ...
                                  <div className="p-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    {category.plans.map((plan) => (
                                      ...
                                      >
                                        ...
                                        <div className="text-sm text-gray-600">{formatZar(plan.monthly)}/mo</div>
                                        <div className="text-xs text-gray-400">or {formatZar(plan.yearly)}/yr</div>
                                      </label>
                                    ))}
                                  </div>
                                </motion.div>
              ...
              >
                {isSubmitting ? 'Processing Your Transfer...' : 'Submit Transfer Request'}
            ...
            <div className="lg:col-span-1 border-t lg:border-t-0 lg:border-l border-gray-100 bg-slate-50/70 p-6 overflow-y-auto">
              <div className="space-y-6">
                <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
                  <h3 className="text-sm font-bold text-gray-900 mb-4 uppercase tracking-wider">Transfer Summary</h3>

                  <div className="space-y-3">
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex flex-col">
                        <span className="text-sm font-bold text-gray-800">Domain Transfer</span>
                        <span className="text-xs text-gray-500">{domain}</span>
                        <span className="text-xs text-green-600 font-medium mt-0.5">Includes 1-year extension</span>
                      </div>
                      <span className="text-sm font-bold text-gray-400 whitespace-nowrap">Price TBD</span>
                    </div>
                  ...
                  <div className="mt-6 pt-4 border-t-2 border-dashed border-gray-200">
                    <p className="text-[10px] text-gray-400 text-center leading-relaxed">
                      Our team will confirm the transfer price and contact you within 1 business day to complete the process.
                    </p>
                  </div>
                </div>

                {/* Transfer checklist reminder */}
                <div className="bg-blue-50 rounded-xl border border-blue-100 p-4">
                  <p className="text-xs font-bold text-blue-900 mb-2 uppercase tracking-wide">Before you transfer</p>
                  <ul className="space-y-1.5">
                    {[
                      'Unlock domain at current registrar',
                      'Obtain your Auth / EPP Code',
                      'Access admin email address',
                      'Disable ID Protection / Privacy',
                    ].map((item) => (
                      <li key={item} className="flex items-start gap-2 text-[11px] text-blue-800">
...
export default DomainTransferOrderModal;
